...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="noise min-h-[100dvh] bg-background text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-30 flex w-[248px] flex-col border-r border-sidebar-border bg-sidebar transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="mt-auto p-5"><div className="rounded-lg border border-sidebar-border bg-background/35 p-3"><div className="flex items-center gap-2 text-[11px] font-bold"><span className="size-2 rounded-full bg-primary shadow-[0_0_8px_hsl(var(--primary))]"/> Console online</div><div className="mono mt-2 text-[10px] text-muted-foreground">LOCAL SESSION / PHASE 6</div></div></div>
    </aside>
