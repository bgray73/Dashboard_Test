...
import { Activity, Boxes, Calculator, ChevronRight, Cog, Command, Gauge, Menu, Network, Radio, Settings, Terminal, X } from 'lucide-react';
  ...
  return <div className="noise min-h-[100dvh] bg-background text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-30 flex w-[248px] flex-col border-r border-sidebar-border bg-sidebar transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex h-[76px] items-center justify-between border-b border-sidebar-border px-6">
        ...
        <button className="md:hidden text-muted-foreground" onClick={() => setOpen(false)} data-testid="button-close-menu"><X size={18}/></button>
      </div>
      ...
      <div className="mt-auto p-5"><div className="rounded-lg border border-sidebar-border bg-background/35 p-3"><div className="flex items-center gap-2 text-[11px] font-bold"><span className="size-2 rounded-full bg-primary shadow-[0_0_8px_hsl(var(--primary))]"/> Console online</div><div className="mono mt-2 text-[10px] text-muted-foreground">LOCAL SESSION / PHASE 1</div></div></div>
    </aside>
    ...
    <main className="min-h-[100dvh] md:pl-[248px]"><header className="flex h-[76px] items-center justify-between border-b border-border bg-background/75 px-5 backdrop-blur md:px-9"><button className="text-muted-foreground md:hidden" onClick={() => setOpen(true)} data-testid="button-open-menu"><Menu size={20}/></button><div className="hidden items-center gap-2 text-xs text-muted-foreground md:flex"><Command size={14}/> <span className="mono">LABOPS /</span><span className="text-foreground">{location === '/' ? 'overview' : location.slice(1).replaceAll('/', ' / ')}</span></div><div className="ml-auto flex items-center gap-4"><span className="mono hidden text-[10px] text-muted-foreground sm:block">UTC {new Date().toISOString().slice(11, 16)}</span><Link href="/settings" className="grid size-8 place-items-center rounded-full border border-border text-muted-foreground hover:bg-secondary" data-testid="link-header-settings"><Cog size={15}/></Link></div></header><div className="p-5 md:p-9">{children}</div></main>
  </div>;
}