...
LabOps Phase 14 uses small in-process schedulers plus PostgreSQL monitoring history, configurable retention with previewed manual cleanup, an incident response workspace, per-device availability reports, bounded operational CSV exports, scheduled maintenance, maintenance audit events, durable webhook delivery state, and a provider-neutral reachability boundary. The Monitoring view reports enabled, due, and maintenance-paused device counts together with its next due time and operator filters. Availability is check-based rather than an SLA-grade time-series calculation and excludes unknown checks. Retention cleanup is bounded to monitoring history and serialized with settings changes in PostgreSQL. Webhook retries are bounded and single-process; there is no external queue or distributed claim mechanism. Native ICMP on the API host is the only active reachability provider; there is no remote collector, SNMP collection, email/SMS, authentication, or RBAC. Hosted containers may not permit ICMP; in that case, failed checks are reported honestly and never mocked as successful.
